...
1. `scripts/build_genre_popularity.py` pusti istrenirani model preko celog realnog dataseta i agregira predikcije po žanrovskom bucket-u koji odgovara 12 žanrova aplikacije (npr. `alt-rock`, `hard-rock`, `punk-rock` → `ROCK`). Rezultat (`models/genre_popularity.json`) je realan, iz modela izveden broj po žanru — ali odražava **globalnu Spotify popularnost strimovanja**, ne lokalnu potražnju za bukingom (primedba mentora).
2. `scripts/build_event_type_fit.py` računa, iz malog ručno prikupljenog dataset-a od 61 realnog lokalnog izvođača (`ml-service/research/`, v. `dataset-description.md`), koliko se određeni žanr stvarno bira za određeni tip događaja (svadba, klupski nastup, festival, korporativni...). Rezultat (`models/event_type_fit.json`) je **lokalno, domenski relevantan** signal — direktan odgovor na primedbu da globalna popularnost žanra ne odražava stvarnu lokalnu potražnju za nastupima.
3. `/recommend` endpoint kombinuje oba signala sa operativnim karakteristikama para događaj–izvođač (podudaranje žanra, budžeta, grada, tipa; ocena; dostupnost; istorijska uspešnost) u transparentnu ponderisanu formulu (v. `app/ml/predictor.py`, `SCORE_WEIGHTS`) — umesto da se ti operativni podaci šalju u klasifikator treniran na nepostojećim (fabrikovanim) booking-labelima, kao u ranijoj verziji rada.
